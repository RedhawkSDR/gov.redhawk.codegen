 ...
 */
package gov.redhawk.ide.codegen.cplusplus.tests;
...
import gov.redhawk.ide.codegen.cplusplus.internal.OctaveEnvMap;
import gov.redhawk.ide.sdr.SdrRoot;
import gov.redhawk.ide.sdr.util.SdrPluginLoader;
import mil.jpeojtrs.sca.spd.SoftPkg;

/**
 * IDE-1407, IDE-1453 Ensure proper paths are used for chalkboard launches
 * We re-use the C++ test's softpkgs here.
 */
public class OctaveEnvMapTest {

	private static final String PLUGIN_ID = "gov.redhawk.ide.codegen.cplusplus.tests";
	private static final String DEFAULT_SDR_PATH = "testFiles/sdr";
	private static final String SUFFIX = "${OssieHome}/lib:${env_var:OCTAVE_PATH}";

	private OctaveEnvMap mapper = new OctaveEnvMap();
	...
	private void checkOctavePath(Map<String, String> map, String[] paths) {
		Assert.assertTrue(map.containsKey("OCTAVE_PATH"));
		String path = map.get("OCTAVE_PATH");

		if (path.endsWith(SUFFIX)) {
			path = path.substring(0, path.length() - SUFFIX.length());
		} else {
			Assert.fail("Required OCTAVE_PATH suffix not found");
		}
		if (path.endsWith(":")) {
		...
		for (int index = 0; index < pathElements.length; index++) {
			if (index >= paths.length) {
				Assert.fail(String.format("OCTAVE_PATH contains more elements (%d) than expected (%d)", pathElements.length, paths.length));
			}
			String errorMsg = String.format("OCTAVE_PATH element %d did not match expected element", index);
	...
	private String getSdrDomLocation() throws URISyntaxException, IOException, CoreException {
		String domUri = SdrPluginLoader.getSdrRoot(PLUGIN_ID, DEFAULT_SDR_PATH).getDomFileSystemRoot().toString();

 ...
 * 
 */
public final class JavaGeneratorProperties {
	public static final String PROP_PACKAGE = "java_package";

	private JavaGeneratorProperties() {
	...
	public static String getPackage(final SoftPkg spd, final Implementation impl, final ImplementationSettings implSettings) {
		...
		for (final Property prop : implSettings.getProperties()) {
			if (JavaGeneratorProperties.PROP_PACKAGE.equals(prop.getId()) && (prop.getValue() != null) && !"".equals(prop.getValue())) {
				pkg = prop.getValue();
				break;
			}
		}
		if (pkg == null) {
			final String basePkg = JavaGeneratorProperties.getBasePackageName(spd);

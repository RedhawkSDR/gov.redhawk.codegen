 ...
 * @since 4.1
 */
public class IdlLibraryProjectNature implements IProjectNature {

	/** The ID of this project nature. */
	public static final String ID = "gov.redhawk.ide.idl.natures.idllibrary";
	...
	public static IdlLibraryProjectNature addNature(final IProject project, final Map<String, String> args, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			for (final String prevNature : prevNatures) {
				if (IdlLibraryProjectNature.ID.equals(prevNature)) {
					return (IdlLibraryProjectNature) project.getNature(IdlLibraryProjectNature.ID);
				}
			}
			final String[] newNatures = new String[prevNatures.length + 1];
			System.arraycopy(prevNatures, 0, newNatures, 0, prevNatures.length);
			newNatures[prevNatures.length] = IdlLibraryProjectNature.ID;
			description.setNatureIds(newNatures);

			project.setDescription(description, monitor);

			IdlLibraryProjectNature.addBuilder(project, args);

			return (IdlLibraryProjectNature) project.getNature(IdlLibraryProjectNature.ID);
		} catch (final CoreException e) {
			RedhawkIdeIdlPlugin.logError("Unable to add IDL library nature to project", e);
		} finally {
			monitor.done();
		}
		return null;

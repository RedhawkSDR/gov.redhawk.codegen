...
public class ConfigureAcTemplate
{
  ...
  protected final String TEXT_3 = ")" + NL + "" + NL + "AC_PREFIX_DEFAULT(${OSSIEHOME})" + NL + "" + NL + "AM_INIT_AUTOMAKE(nostdinc)" + NL + "AC_PROG_CC" + NL + "AC_PROG_CXX" + NL + "AC_PROG_INSTALL" + NL + "AC_PROG_LIBTOOL" + NL + "" + NL + "AM_PATH_PYTHON([2.3])" + NL + "" + NL + "OSSIE_CHECK_OSSIE" + NL + "OSSIE_OSSIEHOME_AS_PREFIX" + NL + "OSSIE_PYTHON_INSTALL_SCHEME" + NL + "" + NL + "AC_CORBA_ORB" + NL + "AC_CHECK_PROG([IDL], [omniidl], [omniidl], [no])" + NL + "if test \"$IDL\" = no; then" + NL + "  AC_MSG_ERROR([cannot find omniidl program])" + NL + "fi" + NL + "AC_LANG_PUSH([C++])" + NL + "PKG_CHECK_MODULES([OMNIORB], [omniORB4 >= 4.0.0])" + NL + "PKG_CHECK_MODULES(OSSIE, ossie >= 1.7.0,,exit)" + NL + "" + NL + "# If you depend on other IDL modules, such as CF or BULKIO add them here" + NL + "# PKG_CHECK_MODULES([BULKIO], [bulkioInterfaces >= 1.7.0])" + NL + "# AC_CHECK_PYMODULE(bulkio.bulkioInterfaces, [], [AC_MSG_ERROR([the python bulkio.bulkioInterfaces module is required])])" + NL + "" + NL + "# Optionally include java support" + NL + "AC_ARG_ENABLE([java], AS_HELP_STRING([--disable-java], [Disable framework java support]))" + NL + "" + NL + "if test \"x$enable_java\" != \"xno\"; then" + NL + "  AC_CHECK_PROG([IDLJ], [idlj], [idlj], [no])" + NL + "  AC_CHECK_PROG([JAVAC], [javac], [javac], [no])" + NL + "  AC_CHECK_PROG([JAR], [jar], [jar], [no])" + NL + "  if test \"$IDLJ\" = no -o \"$JAVAC\" = no -o \"$JAR\" = no; then" + NL + "    HAVE_JAVASUPPORT=no" + NL + "    AC_MSG_WARN([Java support won't be built due to missing dependencies])" + NL + "  else" + NL + "    HAVE_JAVASUPPORT=yes" + NL + "  fi" + NL + "else" + NL + "  HAVE_JAVASUPPORT=no" + NL + "fi" + NL + "AC_MSG_CHECKING([for framework java support])" + NL + "AC_MSG_RESULT($HAVE_JAVASUPPORT)" + NL + "" + NL + "AM_CONDITIONAL(HAVE_JAVASUPPORT, test $HAVE_JAVASUPPORT = yes)" + NL + "# End optional java support" + NL + "" + NL + "AC_CONFIG_FILES(Makefile)" + NL + "AC_OUTPUT";

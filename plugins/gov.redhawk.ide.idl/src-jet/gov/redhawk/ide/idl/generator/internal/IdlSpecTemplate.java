...
public class IdlSpecTemplate
{
  ...
  protected final String TEXT_1 = "# By default, the RPM will install to the standard REDHAWK OSSIE root location (/usr/local/redhawk/core)" + NL + "%{!?_ossiehome: %global _ossiehome /usr/local/redhawk/core}" + NL + "%define _prefix %{_ossiehome}" + NL + "Prefix: %{_prefix}" + NL + "" + NL + "# Point install paths to locations within our target OSSIE root" + NL + "%define _sysconfdir    %{_prefix}/etc" + NL + "%define _localstatedir %{_prefix}/var" + NL + "%define _mandir        %{_prefix}/man" + NL + "%define _infodir       %{_prefix}/info" + NL + "" + NL + "# Assume Java support by default. Use \"rpmbuild --without java\" to disable" + NL + "%bcond_without java" + NL + "" + NL + "Summary: The ";
  protected final String TEXT_2 = " library for REDHAWK" + NL + "Name: ";
  protected final String TEXT_3 = "Interfaces" + NL + "Version: ";
  protected final String TEXT_4 = NL + "Release: 1%{?dist}" + NL + "License: None" + NL + "Group: REDHAWK/Interfaces" + NL + "Source: %{name}-%{version}.tar.gz " + NL + "BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-buildroot" + NL + "Requires: redhawk >= 1.7" + NL + "BuildRequires: redhawk >= 1.7" + NL + "BuildRequires: autoconf automake libtool" + NL + "BuildRequires: omniORB" + NL + "BuildRequires: python omniORBpy omniORBpy-devel" + NL + "%if %{with java}" + NL + "BuildRequires: java-devel" + NL + "%endif" + NL + "" + NL + "%description" + NL + "Libraries and interface definitions for ";
